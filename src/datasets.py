...
class TransformOnImg:
    ...
    def __init__(self, mode):
        ...
        self.trsfm_train = transforms.Compose([
            transforms.ToPILImage(),
            transforms.Resize(256),
            transforms.RandomResizedCrop(size=224, scale=(0.2, 1.)),
            transforms.ToTensor(),
...
class HPADataset(Dataset):
    ...
    def __init__(self, data_dir, mode, batch_size, bag_size, classes=10, shuffle=False):
        file_path = '/workspace/workspace/self/plot/visualize/image.txt'
        ...
        # pretrain_id, train_sids, val_sids, test_sids = split_train_val_test(sids)
        # self.sids是包，每个包里面有一批数据
        # self.db 是蛋白质id的包 对应的包的图像和标签， sids就是蛋白质id包
        # pretrain 和 train 1个sid 的每张图片对应一个包
        if mode == 'pretrain':
    ...
    def filter_top_cv(self, k=10, csv_file="/workspace/workspace/self/enhanced.csv"):
        ...
        # top_cv = [x for x in count if x not in top_cv]
    ...
    def __getitem__(self, index):
        if self.mode == "pretrain":
            ...
            for idx in range(index * self.batch_size, (index + 1) * self.batch_size):
                ...
                for imgpth in sid_imgs:
                    ...
                    img_basic1 = self.transform(img, use_aux=True)
                    # img_basic2 = self.transform(img)
                    img_basic2 = self.transform(img, use_aux=True)
            ...
            n_b, n_c, n_w, n_l = imgs_basic1.shape
        ...
        # 无语，相当于把每个batch的取出来，然后填充 max_bag*batch 的值，其余都是0
...
def makeup_pretrain_dataset(data_dir, batch_size, bag_size, epoch=1, shuffle=False, classes=10, num_parallel_workers=4):
    ...
    ds = DataLoader(pretrain_dataset, batch_size=1, num_workers=num_parallel_workers, shuffle=False)
    return ds
...
def makeup_dataset(data_dir, mode, batch_size, bag_size, epoch=1, shuffle=False, classes=10, num_parallel_workers=4):
    dataset = HPADataset(data_dir=data_dir, mode=mode, batch_size=batch_size, bag_size=bag_size, shuffle=shuffle,
                         classes=classes)
    if mode == "train":
        ds = DataLoader(dataset, 1, num_workers=num_parallel_workers, shuffle=False)
    else:
        ds = DataLoader(dataset, 8, num_workers=num_parallel_workers, shuffle=False)
    return ds


if __name__ == "__main__":
    data_dir = "/workspace/workspace/self/hpa"
    save_checkpoint_path = "/workspace/workspace/self/ckpt"
    log_path = "/workspace/workspace/self/log"
